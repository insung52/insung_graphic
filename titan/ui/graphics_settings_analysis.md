...
2026-09-03 / 조사·설계 완료(구현은 별도 문서) / Graphics 탭 전수 조사 — `DumpCVars` 실측으로 제약
확정(§0-2 F), 채택 구조 설계(§9), 확정 스코프(§10), 프리셋 설계(§12).

> **▶ 구현 경과·WBP 계약·검증 상태는 `ui/2026-09-10_graphics_settings_implementation.md`.**
> 이 문서는 "무엇이 가능하고 왜 그런가"(조사·설계)까지만 다룬다. 실제로 무엇을 만들었고 무엇이
> 남았는지는 그쪽을 볼 것 — 아래 §11/§12의 구현 관련 기록도 그 문서에 정리·갱신돼 있다.
