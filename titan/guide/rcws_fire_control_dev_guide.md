> ⚠️ [guide/ 이관 시 경고, 2026-08-31] 2026-07-12~14 기준 작성 — 이후 LIG 프로토콜 연동
> (`protocol/ugv_rc_feature_gap_analysis.md`, `selfdefense_rc_feature_gap_analysis.md`)으로
> 발사모드/안전스위치/장전 등 상당 부분이 새로 배선됨. 최신 동작은 그 문서들 우선 참고.
...
안정화(2축 안정기), 목표 자동 조준. (발사 이펙트/반동은 최초 스펙에선 범위 밖이었으나
2026-07-14에 실제로 구현됨 — 13절 참고.)

핵심 설계 결정(사용자 확정 사항):
- 안정화는 on/off 가능, 최대 보정 속도도 커스터마이즈 가능
- 표적 선정 기준: 차체 전방 원뿔(기본 좌우 45도) 안의 최근접 적 우선, 없으면 전체 최근접 적
  (2026-09-01 추가 — 그 전까지는 무조건 전체 최근접)
...
`Faction == Enemy`인 것만 대상으로, 총구 위치(`GetMuzzleWorldLocation()`) 기준 거리로
하나를 고름. 자동조준(`UpdateAutoAim`)과 조준점 UI(`UpdateAimPointForUI`)가 공유해서 씀 —
"지금의 그 표적"이 항상 같은 정의를 갖도록.

**전방 우선순위 (2026-09-01 추가, 사용자 요청)** — 한 번의 순회에서 후보를 두 갈래로 모음:

- `NearestInArc` — 차체 전방 원뿔 안에 있는 적 중 최근접
- `NearestOverall` — 기존과 동일한 전체 최근접

전방 후보가 있으면 그걸, 없으면 전체 최근접으로 폴백. 즉 **"전방에 적이 있으면 무조건
그쪽부터, 없으면 예전 그대로"**.

```cpp
bool  bPrioritizeForwardArcTargets = true;   // 끄면 2026-09-01 이전과 완전히 동일
float ForwardPriorityHalfAngleDegrees = 45.f; // 반각 — 45면 좌우 각 45도, 총 90도 원뿔
```

구현 주의점 두 가지:

- 차체 전방은 `UpdateSearchSweep`과 똑같이 `Owner->GetActorRotation().Yaw`를 그대로 씀.
  `InitialMountYawOffsetDegrees`(마운트-차체 상대 오프셋)를 더하면 안 됨 — 8.4절의
  2026-07-28 버그와 정확히 같은 함정.
- 판정은 **XY 평면에 투영**해서 함. 3D forward를 그대로 쓰면 차체가 경사에서 피치/롤로
  기울 때마다 같은 적이 원뿔에 들어왔다 나갔다 하며 표적이 깜빡임.

**스티키 표적과의 상호작용(중요)**: `UpdateAutoAim`의 스티키 표적 로직이 상위에 있어서,
이 우선순위는 **표적을 새로 잡는 순간에만** 적용됨. 이미 후방의 적을 물고 있는 상태에서
...
- `DetectedTargets`에서 **연속으로 사라진 시간** > `TargetRetentionGraceSeconds`(기본 1.0초)

```cpp
UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Fire Control|Auto-Aim")
float TargetRetentionGraceSeconds = 1.f;   // 0이면 2026-09-01 이전 동작(즉시 전환)
```

**왜 유예가 필요했나** — 예전 유지 조건은 "현재 타겟이 이번 스캔의 `DetectedTargets`에
있는가"였는데, 그 목록은 0.1초마다 신뢰도 히스테리시스(획득 0.6 / 상실 0.5,
`ConfidenceLerpSpeed` 1.5)로 드나든다. 차폐 샘플 3발 중 1발만 막혀도 `VisibleFraction`이
0.33이 되어 **0.33초면 목록에서 탈락**한다. 그래서 적이 나무 뒤로 반쯤 들어갔다 나오기만 해도
타겟이 튀었고, 타겟이 바뀌면 `LockOnGaugeValue`가 0으로 리셋되므로 게이지가
`LockOnGaugeChargeSeconds`를 못 채우고 계속 초기화 — **영영 발사하지 않는 상태**가 됐음
(2026-09-01 사용자 리포트: "적 발견하고 쏠 준비할 때 너무 갈팡질팡함").

**왜 락온 게이지를 기준으로 하지 않았나** — 최초 아이디어는 "락온 게이지가 0이 되면 전환"
이었는데, 게이지는 `bAligned`(카메라 `AutoAimLockToleranceDegrees` + 총구
`AutoFireTurretAlignmentToleranceDeg` 둘 다 만족)일 때만 차오른다. 타겟을 새로 잡고 포탑이
슬루하는 동안은 정렬 전이라 **게이지가 계속 0** — 정작 타겟이 튀는 그 구간을 못 덮는다.
그래서 정렬과 무관한 "탐지 유실 시간"을 기준으로 잡았음.

**부수 효과**: 유예 동안에는 표적이 안 보여도 계속 조준·발사한다(엄폐물 뒤로 숨은 지점을
계속 겨눔). 최대 `TargetRetentionGraceSeconds`만큼이며, 실제 운용에서 부자연스럽지 않다고
판단해 그대로 둠. "안 보이면 사격 중지"까지 원하면 발사 게이팅에 `bTargetDetectedNow`를
추가로 물리면 됨.

**주의 — 이것만으로 안 풀리는 경우**: 표적을 고정해도 포탑이 물리적으로 못 따라가면
...
`SearchZoomLevel`은 2026-09-01에 0.5 → **1.0**으로 바뀜(사용자 요청) — 표적이 없을 때
0.5배로 줌아웃해 넓게 보던 동작을 없앰. 이제 탐색 중엔 그냥 기본 배율을 유지함.

`BP_UGV_Vehicle_new`는 스윕 값 셋을 오버라이드해 놓았음: `SearchSweepHalfRangeDegrees` 30,
`SearchSweepSpeedDegPerSec` 5, `MaxAutoZoomChangeRatePerSecond` 1. (`SearchZoomLevel`과
`ZoomReferenceMeters`는 오버라이드 없이 C++ 기본값을 그대로 씀.)
...
```cpp
bool bAutoZoomEnabled = false;   // 2026-09-01 추가. false면 UpdateAutoZoom이 즉시 no-op
```

UGV 통제기 프로토콜에 줌 배율 조정이 없어서 실제 운용상 배율이 저절로 바뀔 이유가 없고,
조준할 때마다 화면 배율이 요동치는 게 과하다는 판단(사용자 확정, 2026-09-01). 끄면
`RCWS->ZoomLevel`이 기본값 0.5에 그대로 머무름.

아래 8.5.1의 로직과 값들은 **지우지 않고 남겨둔 것** — 에디터에서 `bAutoZoomEnabled`
체크박스만 다시 켜면 리빌드 없이 예전 동작으로 즉시 복귀함.

#### 8.5.1 (비활성 상태) 거리비례 계산식

```cpp
TargetZoom = RangeMeters / ZoomReferenceMeters;   // ZoomReferenceMeters 기본 125(2026-09-01: 100→125)
TargetZoom = FMath::Max(TargetZoom, SearchZoomLevel);  // 2026-09-01 하한
```

2026-09-01 조정(사용자 요청, "자동 줌이 너무 심함"):

- `ZoomReferenceMeters` 100 → **125**. 최대 배율(`MaxZoomLevel` 16)에 닿는 거리가
  1600m → **2000m**(16 × 125m)로 밀림.
- **하한 `SearchZoomLevel` 추가**. 이게 없으면 `ZoomReferenceMeters`보다 가까운 적에서
  비율이 1.0 미만으로 떨어져, "넓게 보는 걸 없애 달라"는 요청과 정반대로 적이 코앞에 있을 때
  오히려 `MinZoomLevel`(0.5)까지 줌아웃함. 수동 줌은 이 함수를 안 타므로 0.5배 단계는 그대로 사용 가능.

거리별 결과 (`MinZoomLevel` 0.5 / `MaxZoomLevel` 16 클램프 포함):

| 거리 | 이전 (ref 100, search 0.5) | 현재 (ref 125, search 1.0) |
|---|---|---|
| 표적 없음 | 0.5배 | **1.0배** |
| 50m | 0.5배 | 1.0배 |
| 125m | 1.25배 | 1.0배 |
...
> **줌 배율 기준 재정의 (2026-09-01)** — 화면 FOV 계산식이 바뀌었음:
>
> ```cpp
> 화면 FOV = 렌즈 FOV * (ReferenceZoomLevel / ZoomLevel)   // 예전: 렌즈 FOV / ZoomLevel
> ```
>
> `ReferenceZoomLevel`(신설, 기본 1.0)은 **"화면 FOV가 렌즈 FOV와 정확히 같아지는 배율"**.
> 기본 1.0에서는 예전 식과 완전히 동일하므로 트럭 등 기존 사용처는 영향 없음.
>
> 왜 필요했나 — UGV는 기본 배율을 0.5로 부르기로 했는데(통제기 프로토콜에 줌 조정이 없어서
> 기본값이 곧 상시 화면), 예전 식에서는 0.5가 **문자 그대로 화면을 2배로 넓혀버렸음**.
> 처음엔 렌즈 초점거리를 2배(10→25.53mm)로 늘려 상쇄했는데, 그러면 "렌즈에 넣은 값"과
> "실제 보이는 화각"이 계속 어긋나서 튜닝할 때마다 암산이 필요했음(사용자 지적). `ZoomLevel`과
> `ReferenceZoomLevel`을 같은 값으로 두면 그 어긋남이 사라짐 — **렌즈에 설정한 값이 곧 화면**.
>
> 차량별 설정 — **UGV·트럭 둘 다 같은 규칙으로 통일**(2026-09-01):
>
> | | ReferenceZoomLevel | ZoomLevel 기본 | 렌즈 | 렌즈 FOV |
> |---|---|---|---|---|
> | UGV (`BP_UGV_Vehicle_new`) | 0.5 | 0.5 | 10mm | 99.82° |
> | 트럭 (`BP_TitanTruck`) | 0.5 | 0.5 | **24mm → 10mm** | 99.82° |
>
> 배율별 화면 FOV (두 차량 동일):
>
> | ZoomLevel | 화면 FOV |
...
> | 16.0 | 3.12° |
>
> **UGV는 기본 화면이 변경 전과 정확히 동일**(초점거리를 원래 10mm로 되돌렸으므로 99.82° 그대로).
> **트럭은 화면이 실제로 넓어짐** — 예전 기본 52.67°(24mm) → 지금 99.82°(10mm). 렌즈를 UGV와
> 통일하라는 요청에 따른 의도된 변경이며, 배율 재정의 자체와는 별개의 시각적 변화임.
>
> 부수 효과 — `MinZoomLevel`(0.5)과 `ReferenceZoomLevel`(0.5)이 같아지면서 **화면 FOV가 렌즈
> FOV보다 넓어질 수 없게 됨**. 즉 170° 안전 클램프가 이제 구조적으로 도달 불가능함.
>
> `ManualZoomTargetIndex`는 상수로 두면 안 됨 — Remote 모드 진입 순간 아무도 안 눌렀는데
> 그 인덱스 배율로 램프해버리는데 차량마다 기본 배율이 다르기 때문. `BeginPlay`의
> `SyncManualZoomIndexToCurrentZoom()`이 RCWS의 실제 현재 배율과 **로그2 공간에서** 가장 가까운
> 단계로 맞춤(선형 거리로 재면 배수 눈금 특성상 큰 배율 쪽으로 쏠림).
...
회전음이 UGV 엔진음(약 155m)보다 10배 멀리(약 1.5km) 퍼져서 2026-09-01에 300cm /
4700cm(소멸 약 50m)로 분리·축소함 — 총성과 달리 모터 기계음은 멀리 갈 이유가 없음.
상세는 `sfx_vfx/2026-09-01_barrel_spin_audio_attenuation.md`.
...
## 13. 탄 퍼짐 · 반동 (2026-07-14 구현, 2026-09-01 튜닝)

최초 스펙에선 "범위 밖"이었으나 2026-07-14에 구현됨. 전부 `URCWSFireControlComponent::Fire()`
안에서 처리되고, 값이 0이면 완전한 no-op이라 끄면 예전 동작 그대로임.

### 13.1 탄 퍼짐 (`SpreadAngleDegrees`)

```cpp
FireDirection = (SpreadAngleDegrees > 0.f)
    ? FMath::VRandCone(MuzzleDirection, FMath::DegreesToRadians(SpreadAngleDegrees))
    : MuzzleDirection;
```

- **반각(원뿔 반지름)** 임에 주의 — `VRandCone`의 2번째 인자가 half-angle. 분포는 원뿔 안
  **균등**(가우시안 아님)이라 같은 각도라도 실총보다 흩어져 보임.
- 머즐 이펙트/`MuzzleDirection` 자체는 안 건드리고 **실제 발사되는 속도 방향에만** 적용
  (실총도 총구 화염은 조준선 그대로, 탄착만 퍼짐).
- **서버에서 한 번만 굴려** `Multicast_FireEffects` 파라미터로 넘김 — 클라마다 따로 굴리면
  서버 판정 방향과 시각적으로 어긋남.
...
|---|---|---|---|---|---|
| 0.10° | 0.17m | 0.35m | 0.52m | 0.87m | 1.75m |
| **0.20°** | **0.35m** | **0.70m** | **1.05m** | **1.75m** | **3.49m** |
| 0.30° | 0.52m | 1.05m | 1.57m | 2.62m | 5.24m |
| 0.50° | 0.87m | 1.75m | 2.62m | 4.36m | 8.73m |

### 13.2 반동 — 두 갈래

**차체 임펄스 (`RecoilImpulseMagnitude`)** — 루트가 물리 시뮬레이션 중이면 총구 위치에
발사 반대 방향 임펄스. 3톤급 차체는 브레이크 없이는 회전이 아니라 **뒤로 미끄러지는**
것으로 흡수돼서, 크게 올리면 "차체가 밀리는" 원치 않는 느낌이 남 — 그래서 이 값은 올리지
않고 아래 마운트 킥으로 체감을 만드는 게 원칙.

**마운트 킥 (`RecoilMountKickPitchDegrees` / `RecoilMountKickYawJitterDegrees`)** — 차체
물리와 무관하게 항상 즉시 느껴짐. 발당 `rand(0, Pitch)` / `rand(-Yaw, +Yaw)`를
`AddPanTiltInput`으로 **즉시** 마운트에 반영하고, 같은 양을 `RecoilOffsetPitch/Yaw`에
누적해서 `UpdateRecoilKick()`이 `RecoilRecoverySpeed`(기본 15, `FInterpTo` 지수 감쇠)로
0까지 되돌림. `AC_WeaponTank`가 있으면 `ApplyTurretRecoilKick`으로 포탑/포신에도 같은 킥.

지속 사격 시 정착 오프셋 ≈ (Pitch/2 × 초당 발사수) / `RecoilRecoverySpeed`.
1200RPM(20발/s) 기준:

| MountKickPitch | 평균 입력 | 정착 오프셋 |
|---|---|---|
| 0.5° | 5.0°/s | 약 0.33° |
| **0.8°** | **8.0°/s** | **약 0.53°** |
| 1.0° | 10.0°/s | 약 0.67° |

자동조준/자동사격 중엔 `UpdateAutoAim`이 매 틱 오차를 다시 계산하므로 스프링 복귀와 별개로
저절로도 복구됨 — 즉 반동은 "조준이 영구히 틀어지는" 게 아니라 **조준선이 계속 흔들리는**
형태로 나타나고, 그만큼 실질 명중률도 같이 떨어짐.

### 13.3 `BP_UGV_Vehicle_new` 현재 값 (2026-09-01)

자동 사격이 너무 정확하다는 사용자 요청으로 조정. **전부 BP 오버라이드**라 C++ 리빌드 없이
Details 패널에서 바로 튜닝 가능(`SpreadAngleDegrees`는 이번에 처음 BP 값이 생김).

| 프로퍼티 | 이전 | 현재 |
|---|---|---|
| `SpreadAngleDegrees` | 0 (완전 정확) | **0.2°** |
| `RecoilMountKickPitchDegrees` | 0.5° | **0.8°** |
| `RecoilMountKickYawJitterDegrees` | 0.2° | **0.35°** |
| `RecoilImpulseMagnitude` | 1000 | 1000 (유지 — 13.2절 이유) |
| `RecoilRecoverySpeed` | 15 | 15 (유지) |

## 14. 발사 모드 (단발 / 점사 / 연사)

`RC_FireMode`(`RCWS->GetCurrentData().FireMode`)에 따라 **한 트리거 사이클당 발수**를 제한함.
사이클 **내부** 발사 간격은 `ShotIntervalSeconds`(= `FireRateRPM` 유래)가 담당.

| 모드 | 사이클당 발수 |
|---|---|
| Single | 1 |
| Burst | `BurstRoundCount` (기본 3) |
| Auto | 무제한 |

`ShotsFiredThisTrigger`가 카운터이고, `bWantsToFire`의 **false→true 엣지**에서 0으로 리셋됨.

### 14.1 자동사격에는 "뗄 트리거"가 없다 (2026-09-01 버그 수정)

위 엣지 리셋은 수동 트리거에서는 맞지만, **자동사격에서는 성립하지 않는다**:
...
락온이 유지되는 동안 계속 true라 엣지가 **락온 시작 시점 한 번뿐**. 그래서 자동사격 +
단발/점사면 한 사이클만 쏘고 락온이 풀릴 때까지 영영 멈췄음(2026-09-01 사용자 리포트:
"타겟이 바뀌지 않는 한 한 번만 쏘고 다시는 안 쏨").

수정 — 자동사격일 때만 시간 기반으로 사이클을 재시작:

```cpp
UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Fire Control|Weapon")
float AutoFireCycleIntervalSeconds = 1.f;   // 사이클 사이 간격
```

`bManualFireHeld`면 이 로직을 타지 않음 — 수동은 조작자가 직접 떼었다 당기는 게 맞는 동작이라
"쥐고 있으면 한 발만"이라는 의미를 그대로 유지해야 함. Auto 모드는 사이클 제한 자체가 없어
무영향. 상세는 `rcws/2026-09-01_autofire_single_burst_stuck_bug.md`.
