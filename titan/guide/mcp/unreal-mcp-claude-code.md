> ⚠️ [guide/ 이관 시 경고, 2026-08-31] 작성 시점 불명(이관 문서) — Claude Code/unreal-mcp
> 자체가 계속 업데이트되니 설정 방법이 바뀌었을 수 있음.
...
- **스플라인 포인트를 써넣어도 `UpdateSpline()`이 안 불린다**(2026-09-02 확인). 좌표는 들어가지만
  ...
  **1.0m**(=손대지 않은 기본 2포인트 스플라인)를 반환한다. 같은 컴포넌트에 `bClosedLoop`를
  `True`→`False`로 토글하면 `PostEditChangeProperty`가 `UpdateSpline()`을 태워서 해결.
  쓴 뒤 `SplineCurves.reparamTable.points[-1].inVal / 100`으로 실제 길이를 검증할 것.
  레벨을 저장 후 다시 열면 `PostLoad()`가 알아서 고치므로 **만든 그 세션에서만** 나오는 증상이다.
  주의: 블루프린트 액터의 스플라인에 쓰면 컨스트럭션 스크립트 재실행으로 아키타입 값이 돌아온다
  (`BP_UGV_Vehicle_new`의 `TrackPath_L/R`처럼) — 의도치 않은 컴포넌트는 애초에 건드리지 말 것
