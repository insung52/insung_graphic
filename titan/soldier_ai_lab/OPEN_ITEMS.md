...
| 접두 | 뜻 | 열림 | 해결 |
|---|---|---|---|
| **C** | 측정해야 아는 것 (실험 필요) | 44 (중 1건은 **의도적 미해결**) | 21 |
| **D** | 설계 백로그 (해당 단계에서 채움) | 10 | 0 |
| **Q** | 사용자 결정 필요 | 3 | 9 |
| **R** | 추가 조사 | 5 | 2 |
| **U** | GASP 미확인 | 2 | 5 |
| **W** | 분석에서 파생된 작업 | 21 | 3 |
...
**`ai/2026-09-13_engagement_and_cover.md`**
...
| **C-87** | **교전·엄폐 튜닝값 전체** | `SuppressiveRadiusCm 500` · `LaneToleranceCm 200` · `FriendlyClearanceCm 150` · `MinCertaintyToEngage 0.25` · `SuppressiveReserveFraction 0.4` · `MoveImprovementMargin 0.35` · 엄폐 링 `1200cm × 12` · `HeightSamples 3`. **C-82와 같은 계열의 위험**(P18·P30) | AI 튜닝 |
...
| **D10** | ★★ **목표·임무 개념 자체가 없다** — 사수할 곳도, 뺏을 땅도, 지킬 것도 없다. 엄폐 층이 **노출만 최적화**하므로 병사들이 **무모하게 앞으로 기어 나간다**. AI 층에서 **다음 큰 조각**이다 | **다음 작업** |
...
| **W18** | ★ **데미지·체력·사망이 없다 — 병사는 죽지 않는다** | 그래서 **교전이 끝나지 않고**, "적이 줄어든다"가 인지에도 교전에도 없다. 피격 반응 클립 19개는 반입돼 있으나 배선이 없고(`IMPLEMENTED.md` 6절) 래그돌은 GASP에 이미 있다 |
| **W19** | **린 축과 블라인드 파이어 축을 AI가 몰지 않는다 — 다리는 이미 놓여 있다** | `AITargetLean` · `AITargetBlindFireH/V` 가 `BP_SoldierCharacter`에 있고 `SelectFloat` 배선까지 끝났다. **넣는 쪽이 없을 뿐**이다. 판단이 생기면 변수에 값만 쓰면 된다. `ai/2026-09-13_ai_bridge_and_scene.md` 1.2절 |
| **W20** | **경로의 노출을 안 본다** | 엄폐 후보는 **도착지의 노출**만 평가한다. 그래서 병사가 더 나은 엄폐로 가려고 **적의 정면을 가로지를 수 있다**. `MoveImprovementMargin`은 "건너는 값"을 상수로 물릴 뿐 **어디를 건너는지는 모른다** |
| **W21** | **엄폐 후보가 한 반경의 링 하나다** | `SearchRadiusCm 1200` 위의 `CandidateCount 12`. **2m 앞의 완벽한 자리는 후보가 된 적이 없다.** 링을 여러 개로 늘리는 것은 곧 트레이스 예산 문제가 된다 → [C-83]과 엮인다 |
