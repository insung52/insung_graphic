...
> ⚠️ **2026-09-10부로 더 이상 공유하지 않는다.** 적군 스켈레톤이 수정되면서 애니메이션이
> `AS_Enemy/`로 분리됐고, 적군은 전용 블렌드스페이스(`BS_enemy_kadex`,
> `BS_enemy_kadex_Crouch`)를 쓴다. 이 문서의 gait 계산 구조(2절)와 앵커 300/600 규칙은
> 적군 블렌드스페이스에도 동일하게 적용했으므로 그대로 유효하다.
> 경위는 `2026-09-10_ally_enemy_animation_split.md` 참고.
